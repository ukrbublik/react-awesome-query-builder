...
describe("drag-n-drop", () => {
  it("should move rule after second rule", () => {
    with_qb(
      configs.simple_with_number,
      inits.with_group,
      "JsonLogic",
      (qb, onChange, { expect_queries }) => {
        ...
        expect_queries(["(num == 1 && num == 2)", "(num == 2 && num == 1)"]);
      }
    );
  });
  ...
  it("should move rule into group", () => {
    ...
    do_test(
      ...
      (config, value, onChange, { expect_queries }) => {
        expect_queries([
    ...
    do_test(
      configs.simple_with_number_without_regroup,
      inits.with_numbers_and_group,
      (_config, _value, onChange, _tasks) => {
        sinon.assert.notCalled(onChange);
      }
    );

    do_test(
      configs.simple_with_number_max_nesting_1,
      inits.with_numbers_and_group,
      (_config, _value, onChange, _tasks) => {
        sinon.assert.notCalled(onChange);
      }
