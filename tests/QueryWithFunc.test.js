...
describe("query with func", () => {
  describe("loads tree with func LOWER from JsonLogic", () => {
    export_checks(
      configs.with_funcs,
      inits.with_func_tolower_from_field,
      "JsonLogic",
      {
  ...
  it("set function for number", () => {
    with_qb(
      ...
      (qb, onChange, { expect_jlogic }) => {
        ...
        qb.find(
          ".rule .rule--value .widget--widget .rule--func select"
        ).simulate("change", { target: { value: "LINEAR_REGRESSION" } });
        qb.find(
          ".rule .rule--value .widget--widget .rule--func--args .rule--func--arg"
        )
          .at(2)
          .find("input")
  ...
  describe("loads tree with func LINEAR_REGRESSION", () => {
    export_checks(
      configs.with_funcs,
      inits.with_func_linear_regression,
      "default",
      {
        query: "num == (2 * 3 + 0)",
