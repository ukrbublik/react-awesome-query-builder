...
describe("vanilla widgets interactions", () => {
  it("change number value", () => {
    with_qb_skins(
      ...
      (qb, onChange, { expect_jlogic }) => {
        qb.find(".rule .rule--value .widget--widget input").simulate("change", {
          target: { value: "3" },
        });
        expect_jlogic([null, { and: [{ "==": [{ var: "num" }, 3] }] }]);
      }
    );
  });
  ...
  it("change text value", () => {
    with_qb_skins(
      ...
      (qb, onChange, { expect_jlogic }) => {
        ...
        expect_jlogic([null, { and: [{ "==": [{ var: "str" }, "def"] }] }]);
      }
    );
  });
  ...
  it("change bool value", () => {
    with_qb(
      ...
      (qb, onChange, { expect_jlogic }) => {
        qb.find(
          ".rule .rule--value .widget--widget input[value=false]"
  ...
  it("change slider value", () => {
    with_qb(
      ...
      (qb, onChange, { expect_jlogic }) => {
        qb.find(
          ".rule .rule--value .widget--widget input[type='range']"
  ...
  it("change time value", () => {
    with_qb(
      ...
      (qb, onChange, { expect_jlogic }) => {
        qb.find(
          ".rule .rule--value .widget--widget input[type='time']"
