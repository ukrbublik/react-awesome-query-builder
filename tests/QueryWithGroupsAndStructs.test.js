...
describe("query with !struct and !group", () => {
  describe("import", () => {
    it("should work with value of JsonLogic format", () => {
      with_qb_skins(
        configs.with_struct_and_group,
        inits.with_struct_and_group,
        "JsonLogic",
        (qb) => {
  ...
  describe("export", () => {
    export_checks(
      ...
      {
        query:
          '((results.slider == 22 && results.stock == true) && user.firstName == "abc" && !!user.login)',
        queryHuman:
          '((Results.Slider == 22 AND Results.In stock) AND Username == "abc" AND User.login IS NOT EMPTY)',
        sql:
          "((results.slider = 22 AND results.stock = true) AND user.firstName = 'abc' AND user.login IS NOT EMPTY)",
        mongo: {
          results: {
            $elemMatch: {
              slider: 22,
  ...
  describe("should handle if !group isnot wrapped in #some", () => {
    export_checks(
      configs.with_struct_and_group,
      inits.with_struct_and_group_mixed_obsolete,
      "JsonLogic",
      {
...
describe("query with nested !group", () => {
  describe("with one group rule", () => {
    export_checks(
      configs.with_nested_group,
      inits.with_nested_group,
      "JsonLogic",
      {
  ...
  describe("with two separate group rules", () => {
    export_checks(
      configs.with_nested_group,
      inits.two_rules_with_nested_group,
      "JsonLogic",
      {
  ...
  describe("with two nested groups", () => {
    export_checks(
      configs.with_nested_group,
      inits.with_two_groups_1,
      "JsonLogic",
      {
        query:
          '((results.user.name == "ddd" && results.score == 2) && group2.inside == 33 && results.score == 2)',
        queryHuman:
          '((Results.user.name == "ddd" AND Results.score == 2) AND Group2.inside == 33 AND Results.score == 2)',
        sql:
          "((results.user.name = 'ddd' AND results.score = 2) AND group2.inside = 33 AND results.score = 2)",
        mongo: {
          $and: [
...
describe("query with !struct inside !group", () => {
  describe("export", () => {
    export_checks(
      configs.with_struct_inside_group,
      inits.with_struct_inside_group,
      "JsonLogic",
      {
