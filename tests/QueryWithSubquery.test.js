...
describe("query with subquery and datetime types", () => {
  describe("import", () => {
    it("should work with simple value of JsonLogic format", () => {
      with_qb_skins(
        configs.with_date_and_time,
        inits.with_date_and_time,
        "JsonLogic",
        (qb) => {
  ...
  describe("export", () => {
    export_checks(
      ...
      {
        query:
          '(datetime == "2020-05-18 21:50:01" || (date == "2020-05-18" && time == "00:50:00"))',
        queryHuman:
          '(DateTime == "18.05.2020 21:50" OR (Date == "18.05.2020" AND Time == "00:50"))',
        sql:
          "(datetime = '2020-05-18 21:50:01.000' OR (date = '2020-05-18' AND time = '00:50:00'))",
        mongo: {
          $or: [
            {
...
describe("query with subquery and select types", () => {
  describe("import", () => {
    it("should work with value of JsonLogic format", () => {
      with_qb_skins(
        configs.with_select,
        inits.with_select_and_multiselect,
        "JsonLogic",
        (qb) => {
          expect(qb.find(".query-builder")).to.have.length(1);
        }
