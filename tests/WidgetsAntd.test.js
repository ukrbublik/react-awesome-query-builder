...
describe("antdesign widgets render", () => {
  it("should render select by default", () => {
    with_qb_ant(configs.with_struct, inits.with_nested, "JsonLogic", (qb) => {
      ...
      expect(qb.find(".ant-select-selection-item").at(0).text()).to.equal(
        "user.info.firstName"
      );
    });
  });
  ...
  it("should render cascader", () => {
    with_qb_ant(configs.with_cascader, inits.with_nested, "JsonLogic", (qb) => {
      expect(qb.find(".query-builder")).to.have.length(1);
      expect(qb.find(".ant-cascader-picker-label").text()).to.equal(
        "User / info / firstName"
      );
    });
  });
  ...
  it("should render tree dropdown", () => {
    with_qb_ant(configs.with_dropdown, inits.with_nested, "JsonLogic", (qb) => {
      expect(qb.find(".query-builder")).to.have.length(1);
      expect(
        qb.find(".ant-dropdown-trigger span").at(0).text().trim()
      ).to.equal("firstName");
    });
  });
});
...
describe("antdesign widgets interactions", () => {
  it("change date value", () => {
    with_qb_ant(
      configs.with_all_types,
      inits.with_date,
      "JsonLogic",
      (qb, onChange, { expect_jlogic }) => {
        qb.find("DateWidget").instance().handleChange(moment("2020-05-05"));
        expect_jlogic([
          null,
  ...
  it("change select value", () => {
    with_qb_ant(
      ...
      (qb, onChange, { expect_jlogic }) => {
        ...
        expect_jlogic([null, { and: [{ "==": [{ var: "color" }, "green"] }] }]);
      ...
      (qb, onChange, { expect_jlogic }) => {
        ...
        expect_jlogic([
          null,
          {
            and: [
              {
                all: [{ var: "multicolor" }, { in: [{ var: "" }, ["orange"]] }],
              },
            ],
          },
        ]);
  ...
  it("change treeselect value", () => {
    with_qb_ant(
      configs.with_all_types,
      inits.with_treeselect,
      "JsonLogic",
      (qb, onChange, { expect_jlogic, expect_checks }) => {
        expect_checks({
          query: 'selecttree == "2"',
      ...
      (qb, onChange, { expect_jlogic, expect_checks }) => {
        ...
        expect_jlogic([
          null,
          {
            and: [
              {
                all: [{ var: "multiselecttree" }, { in: [{ var: "" }, ["3"]] }],
              },
            ],
          },
        ]);
  ...
  it("change slider value", () => {
    with_qb_ant(
      ...
      (qb, onChange, { expect_jlogic }) => {
        ...
        w.handleChange(12);
        expect_jlogic([null, { and: [{ "==": [{ var: "slider" }, 12] }] }]);
  ...
  it("change bool value", () => {
    with_qb_ant(
      ...
      "JsonLogic",
      (qb, onChange, { expect_jlogic }) => {
        qb.find("BooleanWidget").instance().handleChange(false);
        expect_jlogic([null, { and: [{ "==": [{ var: "stock" }, false] }] }]);
      }
    );
  });
  ...
  it("change range slider value", () => {
    with_qb_ant(
      ...
      (qb, onChange, { expect_jlogic, expect_checks }) => {
        ...
        expect_jlogic(
          [null, { and: [{ "<=": [19, { var: "slider" }, 42] }] }],
          0
        );

        w.handleChangeFrom(20);
        expect_jlogic(
          [null, { and: [{ "<=": [20, { var: "slider" }, 42] }] }],
          1
        );

        w.handleChangeTo(40);
        expect_jlogic(
          [null, { and: [{ "<=": [20, { var: "slider" }, 40] }] }],
          2
        );
