...
import {defaultValue, deepEqual, getTitleInValueList} from "../utils/stuff";
	...
	if (fieldSettings) {
			if (fieldSettings.listValues && !fieldSettings.allowCustomValues) {
					if (value instanceof Array) {
							for (let v of value) {
									if (getTitleInValueList(fieldSettings.listValues, v) == undefined) {
											return [`Value ${v} is not in list of values`, value];
									}
							}
					} else {
							if (getTitleInValueList(fieldSettings.listValues, value) == undefined) {
