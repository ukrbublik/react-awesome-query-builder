...
export const getNewValueForFieldOp = function (config, oldConfig = null, current, newField, newOperator, changedField = null, canFix = true) {
  ...
  const currentValueSrc = current.get("valueSrc", Immutable.List());
  const currentValueType = current.get("valueType", Immutable.List());
  ...
  newValue = Immutable.List(Array.from({length: operatorCardinality}, (_ignore, i) => {
    let v = undefined;
    if (canReuseValue) {
  ...
  newValueSrc = Immutable.List(Array.from({length: operatorCardinality}, (_ignore, i) => {
    let vs = null;
    if (canReuseValue) {
  ...
  if (showErrorMessage) {
    ...
    newValueError = Immutable.List(valueErrors);
  }
  newValueType = Immutable.List(Array.from({length: operatorCardinality}, (_ignore, i) => {
    let vt = null;
    if (canReuseValue) {
