...
export const defaultOperatorOptions = (config, operator, field) => {
  ...
  if (!operatorConfig)
    return null; //Immutable.Map();
  return operatorConfig.options ? Immutable.Map(
...
export const defaultRuleProperties = (config, parentRuleGroupPath = null) => {
  ...
  let current = Immutable.Map({
    ...
    valueSrc: Immutable.List(),
    //used for complex operators like proximity
    operatorOptions: defaultOperatorOptions(config, operator, field),
  });
  if (showErrorMessage) {
    current = current.set("valueError", Immutable.List());
  }
...
export const defaultGroupProperties = (config) => Immutable.Map({
...
export const defaultRoot = (config) => {
  if (config.tree) {
    return Immutable.Map(config.tree);
  }
  
  return Immutable.Map({
    type: "group",
    id: uuid(),
    children1: Immutable.OrderedMap({ ...getChild(uuid(), config) }),
