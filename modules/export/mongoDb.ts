...
const mongodbFormatItem = (parents, item, config, meta, _not = false) => {
  ...
  const properties = item.get('properties') || Map();
  ...
  if (
    ...
    if (field) {
      ...
      if (hasParentRuleGroup) {
        if (parentPath && fieldName.indexOf(parentPath + '.') == 0) {
          ...
          meta.errors.push(
            `Can't cut group ${parentPath} from field ${fieldName}`
          );
        }
      }
    ...
    let revOperatorDefinition =
      getOperatorConfig(config, reversedOp, field) || {};
