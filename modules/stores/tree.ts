...
const addNewGroup = (state, path, properties, config) => {
  ...
  state = state.setIn(expandTreePath(groupPath, "children1"), Immutable.OrderedMap());
...
const addItem = (state, path, type, id, properties, config) => {
  ...
  if (canAddNewRule) {
    state = state.mergeIn(expandTreePath(path, "children1"), Immutable.OrderedMap({
      [id]: Immutable.Map({type, id, properties})
    }));
  }
  state = fixPathsInTree(state);
  return state;
...
const setField = (state, path, newField, config) => {
  ...
  if (wasRuleGroup && !isRuleGroup) {
    ...
    state = state.setIn(expandTreePath(path, "properties"), Immutable.OrderedMap());
  }
  ...
  if (isRuleGroup) {
    ...
    state = state.setIn(expandTreePath(path, "children1"), Immutable.OrderedMap());
  ...
  return state.updateIn(expandTreePath(path, "properties"), (map) => map.withMutations((current) => {
    ...
    const _currentValueSrc = current.get("valueSrc", Immutable.List());
    const _currentValueType = current.get("valueType", Immutable.List());
...
const setOperator = (state, path, newOperator, config) => {
  ...
  return state.updateIn(expandTreePath(path, "properties"), (map) => map.withMutations((current) => {
    ...
    const currentOperatorOptions = current.get("operatorOptions");
    const _currentValue = current.get("value", Immutable.List());
    const _currentValueSrc = current.get("valueSrc", Immutable.List());
