...
|removeIncompleteRulesOnLoad |false |Remove incomplete rules (w/o value) during validation in `Utils.sanitizeTree()`?
|removeEmptyRulesOnLoad |true |Remove empty rules during validation in `Utils.sanitizeTree()`?
|removeEmptyGroupsOnLoad |true |Remove empty groups during validation in `Utils.sanitizeTree()`?
|removeInvalidMultiSelectValuesOnLoad |true |Remove values that are not in `listValues` during validation in `Utils.sanitizeTree()`? +
