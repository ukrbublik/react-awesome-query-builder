...
|fieldSettings.listValues |+? | |Static list of values. *Required* for `(multi)select` widgets without `asyncFetch`. +
...
|fieldSettings.treeValues |+ | |Static list of values. Required for `tree(multi)select` widgets. +
...
|fieldSettings.allowCustomValues |- |false |For `(multi)select` widgets. If true, user can provide own options in multiselect, otherwise they will be limited to `listValues`
|fieldSettings.showSearch |- |false |For `(multi)select` and `tree(multi)select` widgets. Show search (autocomplete)?
|fieldSettings.treeExpandAll |- |false |For `tree(multi)select` widgets. Whether to expand all nodes by default
|fieldSettings.treeSelectOnlyLeafs |- |true |For `treeselect` widget. Can select only leafs or any node?
|fieldSettings.asyncFetch |-? | |For `(multi)select` widgets. *Required* if `listValues` is not provided. +
...
|fieldSettings.useAsyncSearch |- |false |For `(multi)select` widgets with `asyncFetch` provided. +
...
|fieldSettings.useLoadMore |- |false |For `(multi)select` widgets with `asyncFetch` defined. +
  If true, `asyncFetch` supports pagination.
|fieldSettings.forceAsyncSearch |- |false |For `(multi)select` widgets with `asyncFetch` provided. +
...
|fieldSettings.fetchSelectedValuesOnInit |- |false |For `(multi)select` widgets with `asyncFetch` provided. +
  If true, selected list values will be fetched on load from server to resolve their titles. +
  `asyncFetch` should support format `async (Array selectedValues) => { values: Array }` +
  where `selectedValues` - array of values (like `['yellow', 'green']`) and `values` - list of `{title: string, value: mixed, groupTitle?: string}`
