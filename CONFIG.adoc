...
And `jsonLogic` for http://jsonlogic.com[JsonLogic]. 
...
|sqlFormatOp |- for SQL format | |Function for advanced formatting SQL WHERE query if just `sqlOp` is not enough. +
  ...
  `value` - mixed for `cardinality==1` -or- `Array` for `cardinality>2`
|jsonLogic |+ for http://jsonlogic.com[JsonLogic] | |String -or- function for JsonLogic. +
...
|jsonLogic |+ for http://jsonlogic.com[JsonLogic] | |Func name for JsonLogic
