...
|preferWidgets | | |See usecase at {config3}[`packages/examples/src/demo/config`] for `slider` field. Its type is `number`. +
  ...
  Setting `preferWidgets: ['price']` will render price input (supports thousands separator). +
...
|valueFormat |`YYYY-MM-DD HH:mm:ss` |`date`, `time`, `datetime` |Format to store date/time in query. +
  By default `YYYY-MM-DD` for date, `HH:mm:ss` for time, `YYYY-MM-DD HH:mm:ss` for datetime.
...
|timeFormat | |`HH:mm` |Option for `<TimeWidget>`, `<DateTimeWidget>` to display time in widget. Example: `'HH:mm'`
|use12Hours | |`false` |Option for `<TimeWidget>`. Use AM/PM?
|dateFormat | |`DD.MM.YYYY` |Option for `<DateWidget>`, `<DateTimeWidget>` to display date in widget. Example: `YYYY-MM-DD`
|valueFormat | |`YYYY-MM-DD HH:mm:ss` |Option for `<TimeWidget>`, `<DateWidget>`, `<DateTimeWidget>` to prepare string representation of value to be stored. Example: `YYYY-MM-DD HH:mm`. +
  By default `YYYY-MM-DD` for date, `HH:mm:ss` for time, `YYYY-MM-DD HH:mm:ss` for datetime.
|maxLength | | |Option for `<TextWidget>`, `<TextAreaWidget>`
|maxRows | | |Option for `<TextAreaWidget>`
|labelYes, labelNo | | |Option for `<BooleanWidget>`
|thousandSeparator|  | |Option for `<PriceWidget>`. Defines the thousands separator character (eg. `,`). +
|decimalSeparator|  |`.` |Option for `<PriceWidget>`. Defines the decimal character. +
|decimalScale|  | |Option for `<PriceWidget>`. If defined, it limits the number of digits after the decimal point. +
|prefix|  | |Option for `<PriceWidget>`. Adds the prefix character before the input value (useful for currency). +
|suffix|  | |Option for `<PriceWidget>`. Adds the suffix after the input value (useful for currency). +
|allowNegative|  |true |Option for `<PriceWidget>`. If set to false, negative numbers will not be allowed.
