...
And `celFormatConj`, `celOp`, `celFormatOp`, `celFormatValue`, `celFormatReverse`, `formatCelField`, `celFormatFunc` for building query in https://github.com/google/cel-spec[Google Common Expression Language (CEL)] with `QbUtils.celFormat()` (and back with `QbUtils.loadFromCel()`). +
