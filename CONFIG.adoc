...
[source,javascript]
----
{conjunctions, operators, widgets, types, functions, settings, fields}
----
...
|label | |same as func key |Label to be displayed in functions list +
  (If not specified, fields's key will be used instead)
|formatFunc | |For `isForDisplay==false` - `FUNC(val1, val2)`, for `isForDisplay==true` - `FUNC(arg1: val1, arg2: val2)` |Function for formatting func expression. +
  `(Object args, bool isForDisplay) => string` +
  `args` - map `{<arg name> => <arg value>}``
|sqlFunc | |same as func key |Func name in SQL
|mongoFunc | |same as func key |Func name in Mongo
...
|args.* | | |Arguments of function. Config is almost same as for link:#configfields[config.fields]
...
|args.<arg>.valueSources | |keys of `valueSourcesInfo` at link:#configsettings[config.settings] |Array with values `'value'`, `'field'`, `'func'`, `'const'`. +
  `const` is special for args, requires `defaultValue`
