...
But if you use server-side config, you may need to add your custom functions (eg. `validateValue`) to `ctx` and refer to them in other config sections with name.
...
// zipConfig can be passed to backend
const zipConfig = {
  fields
};

// and loaded from backend with providing ctx
...
const ctx = {
  ...
  W: {
    VanillaButton,
    // ... other widgets proviede with the lib
  },
  utils: {
