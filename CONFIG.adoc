...
TIP: Example: https://github.com/ukrbublik/react-awesome-query-builder/tree/master/packages/examples/demo/config.tsx[`demo config`]
...
|useConfigCompress |false |Set to `true` if you use `Utils.ConfigUtils.compressConfig()` and `Utils.ConfigUtils.decompressConfig()`. See link:#configctx[config.ctx]
|===
...
Required starting from version 6.2.0

Typically you just need to copy it from basic config.

But if you use server-side config (with `settings.useConfigCompress = true`), you may need to add your custom functions (eg. `validateValue`) to `ctx` and refer to them in other config sections with name.
...
You can't just pass function to `validateValue` in `fieldSettings` because config with functions can't be serialized to JSON and then used in SSR. 
But with approach as in this example config will be compressed with `const zipConfig = Utils.ConfigUtils.compressConfig(config, BasicConfig)` to JSON format. It will contain only changes to `BasicConfig`, and `ctx` will be omitted. 
Then on client side it can be decompressed with `Utils.ConfigUtils.decompressConfig(zipConfig, BasicConfig, ctx)`. 
Note than `ctx` with implementation of `validateFirstName` is required to be provided here. 