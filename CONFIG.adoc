...
Section link:#configctx[config.ctx] demonstrates the concept of `zipConfig` which is a special config format that contains only changes against full config and can be serialized to JSON. 
