...
|sqlImport | | |Function to convert given raw SQL value (not string, but object got from `node-sql-parser`) to object of arguments of current function. If given expression can't be parsed into current function, throw an error.
